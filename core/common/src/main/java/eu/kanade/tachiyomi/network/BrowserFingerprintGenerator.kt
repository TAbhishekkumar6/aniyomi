...
class BrowserFingerprintGenerator {
    ...
    fun generateFingerprint(): Map<String, String> {
        ...
        return mapOf(
            ...
            "sec-ch-ua" to buildSecChUa(chromeVersion),
    ...
    fun getDefaultFingerprint(): Map<String, String> {
        val defaultUA = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) " +
    ...
    private fun generateUserAgent(chromeVersion: String, platform: String): String {
        val baseUA = "Mozilla/5.0"
        val webkit = "AppleWebKit/537.36 (KHTML, like Gecko)"
        val chrome = "Chrome/$chromeVersion"
        val safari = "Safari/537.36"

        return when (platform) {
            "Win32", "Win64" ->
                "$baseUA (Windows NT 10.0; $platform) $webkit $chrome $safari"
            "Linux x86_64" ->
                "$baseUA (X11; Linux x86_64) $webkit $chrome $safari"
            "MacIntel" ->
                "$baseUA (Macintosh; Intel Mac OS X 10_15_7) $webkit $chrome $safari"
            else -> getDefaultFingerprint()["User-Agent"]!!
        }
    }

    private fun buildSecChUa(chromeVersion: String): String {
        val version = chromeVersion.split(".")[0]
